...
const MoviesView: React.FC = () => {
    ...
    const handleStatusChange = async (movie: Movie, newStatus: 'active' | 'inactive' | 'scheduled') => {
        try {
            ...
            setDropdownState(null);
            const labels: Record<string, string> = { active: 'activé', inactive: 'désactivé', scheduled: 'programmé' };
            showSuccessToast('Statut mis à jour', `Film ${labels[newStatus]} avec succès`);
        } catch (error) {
            console.error('Failed to update status:', error);
            showErrorToast('Erreur', 'Erreur lors du changement de statut');
        }
    };
    ...
    const handleToggleBoolean = async (movie: Movie, field: 'isTop10' | 'isHero') => {
        ...
        try {
            await moviesService.updateMovie(movie.id, { [field]: newValue });
            setMovies(prev => prev.map(m => m.id === movie.id ? { ...m, [field]: newValue } : m));
            setDropdownState(null);
            const label = field === 'isTop10' ? 'Top 10' : 'Section Héros';
            showSuccessToast(`${label} ${newValue ? 'activé' : 'désactivé'}`);
        } catch (error) {
            console.error(`Failed to update ${field}:`, error);
            showErrorToast('Erreur', 'Erreur lors de la mise à jour');
        }
    };

    const handleSaveMovie = async (movieData: Partial<Movie>): Promise<void> => {
        try {
            if (selectedMovie && modalType === 'edit') {
                ...
                showSuccessToast('Film mis à jour', 'Les modifications ont été enregistrées avec succès');
            } else {
                const created = await moviesService.createMovie(movieData as any);
                setMovies(prev => [created, ...prev]);
                setModalType(null);
                setSelectedMovie(null);
                showSuccessToast('Film publié', 'Le film a été ajouté avec succès');
            }
        } catch (error: any) {
            console.error('Error saving movie:', error);
            const detail = error.message || 'Erreur inconnue';
            showErrorToast('Erreur', detail);
            throw error; // Re-throw so MovieModal resets isSubmitting
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            setModalType(null);
            setSelectedMovie(null);
            showSuccessToast('Film supprimé', 'Le film a été supprimé avec succès');
        } catch (error: any) {

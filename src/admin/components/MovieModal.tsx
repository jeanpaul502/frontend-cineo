...
const MovieModal: React.FC<MovieModalProps> = ({ isOpen, onClose, onSave, movie, prefill }) => {
    const normalizeGenres = (g: unknown): string[] => {
    ...
    useEffect(() => {
        if (movie) {
            ...
            setFormData({
                ...
                titleLogo: prefill.titleLogo || '',
                description: prefill.description || '',
                ageRating: prefill.ageRating || '',
                score: typeof prefill.score === 'number' ? prefill.score : 0,
                section: (prefill.section as any) || 'Tendances',
                genres: normalizeGenres(prefill.genres),
                releaseDate: prefill.releaseDate ? new Date(prefill.releaseDate).toISOString().split('T')[0] : new Date().getFullYear().toString(),
                isTop10: !!prefill.isTop10,
                isHero: !!prefill.isHero,
                status: (prefill.status as any) || 'active',
                scheduledDate: prefill.scheduledDate ? new Date(prefill.scheduledDate).toISOString().split('T')[0] : '',
                videoUrl: prefill.videoUrl || '',
    ...
    }, [movie, prefill, isOpen]);

    // Debounced Search
    useEffect(() => {
        const timer = setTimeout(() => {
            if (searchQuery.trim() && !movie) {
                handleSearch();
            }
        }, 500);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // The error is handled by the parent (MoviesView) via toast
        } finally {

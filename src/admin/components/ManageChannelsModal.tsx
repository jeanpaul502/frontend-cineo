...
const ManageChannelsModal: React.FC<ManageChannelsModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-slate-900 w-full max-w-3xl rounded-2xl border border-slate-700/50 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                {/* Header */}
                ...
                <div className="p-4 sm:p-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
                    <div className="flex justify-between items-center bg-slate-800/30 p-4 rounded-xl border border-slate-700/30">
                        <div className="text-sm font-medium text-slate-400">
                            <span className="text-blue-400">{(playlist?.channels || []).length}</span> chaînes trouvées
                        </div>
                        <button
                            onClick={onAddChannel}
                            className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-sm font-bold transition-all transform active:scale-95 shadow-lg shadow-blue-500/20"
                        >
                    ...
                    <div className="bg-slate-900 border border-slate-700/50 rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[50vh]">
                        <div className="overflow-y-auto flex-1 custom-scrollbar">
                            <table className="w-full text-left text-sm border-collapse">
                                <thead className="sticky top-0 z-10 bg-slate-900 text-slate-400 border-b border-slate-700 shadow-sm">
                                <tr>

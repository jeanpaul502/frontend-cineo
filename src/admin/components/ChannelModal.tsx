    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (Array.isArray(errorMessage)) {
                alert(`Erreur de validation : ${errorMessage.join(', ')}`);
            } else if (errorMessage?.includes('existe déjà')) {
                alert('Cette playlist existe déjà pour ce pays.');
            } else {
                alert(mode === 'add_channel' 
                    ? 'Impossible d\'importer la playlist. Vérifiez l\'URL et réessayez.' 

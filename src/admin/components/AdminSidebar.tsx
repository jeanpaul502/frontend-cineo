...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ user, activeTab = 'overview', onTabChange }) => {
    ...
    const menuItems = [
        ...
        // { id: 'transactions', label: "Transactions", icon: "solar:card-transfer-bold" }, // Disabled
        { id: 'users', label: "Utilisateurs", icon: "solar:users-group-rounded-bold" },
        { id: 'movies', label: "Films", icon: "solar:clapperboard-play-bold" },
        { id: 'series', label: "Séries", icon: "solar:play-stream-bold" },
        { id: 'channels', label: "Chaînes TV", icon: "solar:tv-bold" },
        { id: 'requests', label: "Demandes", icon: "solar:clipboard-list-bold", badge: 12 },
        { id: 'notifications', label: "Notifications", icon: "solar:bell-bold", badge: 5 }
    ];

    return (
        <aside 
            className="fixed left-0 top-0 h-screen w-64 bg-black/95 backdrop-blur-xl border-r border-white/20 p-6 z-20 flex flex-col overflow-hidden"
        >
            {/* Logo */}
